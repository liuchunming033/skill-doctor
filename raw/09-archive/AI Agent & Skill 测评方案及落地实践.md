...
published: false
created: 2026-06-21
description: "万字长文落地Agent评测实践"
tags:
  - "clippings"
ingested: false
---
腾讯程序员 腾讯技术工程 *2026年6月16日 17:33*
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz9070RDBYNhr3QzWLvSeu2TtlXNRJTDeornu86NUqPLb6zUC1s0EI23seetn1TWEPfgXjwA4t3icAI06YWTvwJAibZYvbU8ibbiaOfia4/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=2)
...
> **经验法则** ：能自动化的坚决不找专家；专家时间应 **60% 以上** 花在"校准 LLM 评委"和"诊断异常"上。

### 2.2 五个维度：评什么？

了解了"谁来评"之后，接下来明确"评什么"。我们将 Agent 的测评覆盖拆解为 **五个大类** ，从"做对了吗"到"好用吗"逐层递进：
...
> - **pass^k** ：k 次试验中 **每次都** 通过的概率，衡量稳定性； **pass@k** ：k 次中 **至少 1 次** 通过的概率，衡量峰值能力。
...
> 这是被很多团队忽视但 **极其重要** 的一类。一个 pass@1 高但 token 花 10 倍的方案，在生产上是不可接受的。建议每个任务都带上成本画像。
...
> **pass^k 释义** ：k 次试验中 **每次都** 成功的概率，用于衡量一致性/稳定性。与之对应的 pass@k 是 k 次试验中 **至少 1 次** 成功的概率，用于衡量峰值能力。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907Ykt6HhAqOrKHHUMQp3H67kfkqkyTaFv5Bzt6F6Ve8FF81GuYYKSiba1zAsb8dyYq37zkYibtCqBxILJRO3icRK9X7hRS43dWlTY/640?wx_fmt=png&from=appmsg#imgIndex=3)

### 3.1 设计测评用例集

用例集需要从 **触发 → 核心逻辑 → 产物质量 → 异常容错** 四个场景层层递进，每个场景都包含 **正向** 和 **负向** 用例。
...
| --- | --- | --- | --- | --- |
| trigger-pos-01 | 正向 | "分析用例134263" | skill\_triggered | 标准表述 |
| trigger-pos-02 | 正向 | "帮我看看用例134263的性能数据" | skill\_triggered | 口语化表述 |
| trigger-neg-01 | 负向 | "CPU高负载应该如何排查？" | skill\_not\_triggered | 通用问答，非分析任务 |
| trigger-neg-02 | 负向 | "用例134263是谁创建的？" | skill\_not\_triggered | 涉及用例但非性能分析 |

> **为什么需要负向触发用例？** 只测正例，Agent 可能学会"什么都触发"——过度触发比不触发更难发现。

#### 场景二：核心逻辑用例

验证 Skill 触发后的 **执行过程** 是否按预期步骤进行——调用了哪些工具、顺序是否正确、参数是否合理。
...
> **经验法则** ：核心逻辑用例的数量通常是其他三类场景用例之和的 **2–3 倍** 。如果一个 Skill 有 N 条核心分支路径，至少需要 N 个正向用例 + 关键分支的负向用例。分支过多时，优先覆盖 **高频分支** 和 **易出错分支** 。

#### 场景三：产物质量用例

验证 Skill 的 **最终产物** ——响应内容、输出文件、报告等是否完整、准确、可用。
...
验证 Skill 在 **异常输入、边界条件、环境故障** 下能否合理处理，而非崩溃或静默失败。
...
用例集定义了"测哪些场景"，评分规则则定义了"怎么判对错、怎么算分"。一个好的评分规则需要兼顾 **可解释性** （为什么扣分）和 **区分度** （好坏能拉开差距）。以下是评分规则的 **设计示例** 。
...
设计完用例后，我们只定义了 prompt 和检查规则，但 **并不确定 Agent 实际的执行过程和结果是什么样的** 。因此需要先跑一轮，看到真实的过程和结果，由人工评估是否可接受——如果 OK，就将这轮的过程和结果保留下来，作为该用例的 **预期基线** 。

#### 什么是用例基线？

用例基线是 **单个用例** 执行 1 次后，经人工确认的 **预期过程** 和 **预期结果** 的快照。它回答的是："这个用例，正确的执行应该长什么样？"
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/KVER9adz904Nibksfib4piaYezVISlS8veCMb6I3ibDBRfBd1JmBjstkusLQibL4YLiag8HbJgGPPOvUgKauf2sRTBltUfJa1pXicA62Yf8UF4u9ao/640?wx_fmt=jpeg#imgIndex=4)
...
后续每次测评执行时，将本次执行的过程和结果与用例基线做对比。基线同时服务于 **确定性评分器** 和 **Rubric 评分器** 两种评分方式：
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/KVER9adz907cnAvmnCVyJibQ8cUbRRCVPCxyH6pkhbovJ9GxLIiaicrXHc6Nn3KibUia8nvVp96CX6qWbWJJPMZ7KQR3fpKkEqVnG7hIEufuHnxg/640?wx_fmt=jpeg#imgIndex=5)
...
> 与传统的测试流程不同，能力测评是和Skill开发 **同步启动** 的，Skill开发人员需要在设计之初就确定测评集，在开发过程中不断测评，通过测评结果优化Skill
...
> 根据Anthropic的推荐，对于确定性的任务， **用例集整体通过率** （即所有用例中通过的占比）需要达到 **100%** ，对于非确定性的任务，推荐通过率 **≥ 95%** 。
...
生产/线下环境发现的 Bad Case 是 **最有价值的测评素材** ：
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906glCuZZBKicGZ8URpNibpsnRj4YIDaxMXbLO773TIfbJeh0IuFTvibSS3eam8JPHWpxsm9R03qqiatdKdNfERtibKe0LIOl1dMJw6I/640?wx_fmt=png&from=appmsg#imgIndex=6)

### 4.1 前置依赖：被测 Agent/Skill 的 Trace 输出能力

过程评测的前提是 **能拿到结构化的执行轨迹** 。如果被测 Agent 只输出最终回答，没有可解析的中间过程，那么"工具调用检查"、"过程对比"、"基线对比"等评分器就无从下手。
...
> **建议** ：在 Agent/Skill 设计阶段就将结构化 Trace 输出作为 **标准能力** 纳入，而非事后补救。这不仅服务于测评，也有利于线上问题排查和可观测性建设。

### 4.2 环境隔离

每次测评在 **隔离环境** 中执行，避免状态污染：
...
> **调试技巧** ：0% pass^N（即 N 次试验无一通过）通常说明 **任务定义有问题，而不是 Agent/Skill 能力不行** 。先检查用例描述是否有歧义、评分器是否配置错误、成功标准是否不合理。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907nlf5KKHjt7icdNbgxQIWMgCicsqFfxGu8bcFZhdCFD4m63ibkZHqYnQzoEFSgPGp60OFf1QvEnibyWqtttIbLJLMONF8JDAKwsuU/640?wx_fmt=png&from=appmsg#imgIndex=7)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907pJu7HAzicwrTcHkXankFzMSkUib54I3mfdPeYpEicp63FO1K8NlVIzKqtcDHu62eSO5ibJdZDdaw7FI3PfJ3mkOyiczvkyBQpJzVw/640?wx_fmt=png&from=appmsg#imgIndex=8)
...
采用 **确定性评分 + 模型评分** 两层组合， **满分 100 分扣分制** （沿用第三章 3.2 的通用评分框架）。TPerf 将通用方案中的"任务结果"映射为"关键判定"（压测有效性 + 性能结果），将"过程"细化为"操作步骤"，将"产物质量"细化为"章节内容"逐项对比。
...
明确了评分标准后，接下来看用例集如何组织。TPerf AI 分析 Agent 的用例集按 **性能瓶颈场景** 分组，覆盖 9 大类共 30+ 个用例，每个用例对应一个真实的 TPerf 压测记录。每个用例支持 **多模型对比评测** （7 个模型，每个用例 × 每个模型生成独立的参数化测试）。
...
- **基线会话标识** ：指向人工确认正确的一次分析结果（session\_id + message\_id）
```
task_group: test_cpu
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz905JfSmib3URwxgfHpu8ADegAoCEuydGVLdftdhGjhl6lU9W21QibqV6sN9uLgvyibNTIIQ9CKQ9bBfEfejVMWtZZnnXsRSlUZ8uLw/640?wx_fmt=png&from=appmsg#imgIndex=9)

基线建立核心步骤：

1. **触发分析** ：向 Knot Agent 发送"分析用例{记录ID}"，触发一次完整分析
2. **获取标识** ：记录本次分析的 session\_id 和 message\_id
...
评分标准、用例集、基线三件套就绪后，进入实际执行阶段。TPerf 的测评执行充分利用了 **并发能力** ——多个 Trial 并行触发 Agent、Trial 评分并行、三个评分器并行打分——以在可接受的时间内完成大规模多模型对比测评。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905hMteJFN6PJicHvR36TIVibKF1vXpuHAwNwHChC5w1sCKqongY2LmhRhs4fAe29AEYLWE65libOSmHcvBONN4RtJ25xDMVPib69hI/640?wx_fmt=png&from=appmsg#imgIndex=10)

**单个 Trial 执行流程：  
**

![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907Rf03Y2lEz3PGrlgic79aULQSX8oYYV9R4UZxcwUh1WnPrjwpOBZm4jaFh1j9DMM1swa85tqTzic2ibbmYaicCKq3VwQrPy79qdho/640?wx_fmt=png&from=appmsg#imgIndex=11)
...
TPerf AI 分析 Agent 部署在 **Knot 智能体平台** 上，通过 Knot 的 AG-UI 协议进行交互。

采用 **Knot API 查询** 方式：触发 Agent 分析后，通过轮询等待数据库同步完成，然后获取完整的 AG-UI 事件流。该事件流包含：
...
测试场景基于 **真实的 TPerf 压测记录** ，不使用 mock 数据。压测记录一旦生成即不可变，确保用例 **可复现** 。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_jpg/KVER9adz905Tx41OvAlxkhmHHN6PRPXjwqQlnEkxuj2yqfXgTxY0D4ib2MD0lc7bgZaEyQKPmCDXbEM5NYX0j34leg1wnyBf9CDWNdL163Jo/640?wx_fmt=jpeg#imgIndex=12)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906NyiaInazZybTyhE2hmGmwNgvIdibaibER8ziaibIhnn8guicCtHVibicM0r9TubqZKWJUZr2odXnzdrKc4xRu418ibmnQXxrzBUfjROn0/640?wx_fmt=png&from=appmsg#imgIndex=13)

**测评流水线**

基线确认后进入正式测评环节，采用 **pytest 实现全部测评逻辑，用例管理复用 TCase 平台** ：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/KVER9adz905fib9ia51fHuGsUWbJvPQm6ZJgCYicC52BWXRUiaKibk15u4tISPPTeoENJ8wuZB4UEtS6mP4LwuAP3tR4S39oiaQKzeV0WSmtJs5qg/640?wx_fmt=jpeg#imgIndex=14)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz905vqDLhDdkToEBDB5PbgIML5zNduiatntJO4cQTLBb04dC49NKV7LrOWibIqJ33pm6F10y5ehYD8ibHAUyRGunKDw4Ny0JO7vldA0/640?wx_fmt=png&from=appmsg#imgIndex=15)

> **单用例评分详情** ：展示该用例的基线信息（耗时/Token/费用/步骤数）、Prompt、多次 Trial 的逐项评分（Transcript 扣分、效率扣分、结果扣分）及最终稳定性评分。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906s4Rq4mSDHz0GqFQQ1McnJfj6fLejhZibI1JIHnkiclyv2rRWt8XVicOFwvGpYCCsOguKPQeSbgj6NDtsuGdicv4T0xichyO9hzoibU/640?wx_fmt=png&from=appmsg#imgIndex=16)

> **Trial 对话详情** ：展示单次 Trial 的完整对话 ID、Token 消耗、费用、耗时，以及 Transcript 各维度评分的扣分明细和上限，支持跳转至 Knot 平台查看完整对话历史。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz905BRZ7hcwXXjyYuvXWEsprXbnjH01nAuN2zC1I7D6NTE8bR4QBtC86syLrg1GeMldomApxfiaSfuz4rZ1qtCom0fiaVicdVlzaJsI/640?wx_fmt=png&from=appmsg#imgIndex=17)
...
- **基线配置** 随代码仓库版本化管理，变更可通过 Git 追溯
- **执行结果** 每次执行重新生成，通过流水线归档保留
...
| Anthropic | Evaluating AI agents: Best practices for reliable measurement | [https://www.anthropic.com/engineering/evaluating-ai-agents](https://www.anthropic.com/engineering/evaluating-ai-agents) |
| OpenAI | Eval skills: Build better AI applications through evaluation | [https://developers.openai.com/blog/eval-skills](https://developers.openai.com/blog/eval-skills) |
