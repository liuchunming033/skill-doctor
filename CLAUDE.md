...
- `raw/` (不可变层 - Immutable)：
  - **仅允许 Markdown 文件（.md）进入 raw/**。所有外部来源必须先转换为 Markdown 再导入。
  - **正文只读**。禁止修改正文内容；允许仅更新 frontmatter 中的 ingest 状态字段（如 `ingested`）。
  - **禁止删除此目录下的源文件**。它是事实的唯一真相来源。
...
- `/init-vault`：初始化或补齐 Vault 基础结构（目录、`CLAUDE.md`、`wiki/index.md`、`wiki/log.md`），默认只创建缺失项，不覆盖已有文件。
- `/ingest <路径>`：仅处理指定的 `raw/*.md` 文件；通过 frontmatter 的 `ingested` 字段判断是否跳过。处理完成后必须更新 index 和 log。
...
created: YYYY-MM-DD
updated: YYYY-MM-DD
sources: [source-slug-1, source-slug-2] # raw sources this page draws from
related: [wiki/page1, wiki/page2] # cross-links to other wiki pages
---
```
