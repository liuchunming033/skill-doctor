...
ingested: false
---
楚翎、程知微 阿里云开发者 *2026年6月26日 10:49*

![图片](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1zrDibumtMAMSeVzo6G7NDSWLxSdMPYD9vafu5SMH2EtVz5oM69xfB32xjZPrIQy29ibU2WQLlvqiayA1Tc8VMDP4w9ibSYwDEv5qo/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1wtPul2C7FmgDqU2pcXKOMx4F6ic69aXeicDOwyewKss7bae1LUsOib4eqbbgsljkhO44FEaVLfmPryCq85d2YdNsuN89KKBFaPsA/640?wx_fmt=png&from=appmsg#imgIndex=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1zgqWQQmbtrkErOWwxQUEcdHqxcMI5xfMUbyWQt6WylVs6VHB3t0Js6PIRiahkknbxiagXic7ZYaTxPtPvGTxSWjiaAu3K5LFTVzwg/640?wx_fmt=png&from=appmsg#imgIndex=2)

#### 2.1.2 xxx交易指标召回

用户问"xxx交易指标怎么取"，系统经域推断 → 多路搜索 → 详情验证三步，精准推荐 DIM 层标签表（直接有xxx字段）和 DWD 层明细表（xxx标记字段可自行聚合），并附带判定逻辑、SQL 示例和血缘链路，一次查询给出从选表到取数的完整答案。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1wubtsd9JeQ61UOa7qaBwVs30L8wEu0tPSZbgOokpDGQD94XEfg1ibWSjkNqpMRUaV5x6H4jEicP2wDfL75rGK9qBtBJx6w3ib8V8/640?wx_fmt=png&from=appmsg#imgIndex=3)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1z0Y7d9p6zSxV8IZdS4QnuYYRB7sFEWkyhmkmOXZA6TM7L4LArRt7nJIqMzwYmUSufkdbbgu2TvKiaMQ0DqoPfobtwYU3ulUyD4/640?wx_fmt=png&from=appmsg#imgIndex=4)
...
##### 场景一：数据源切换（data\_source\_switch）
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1z9yeiceBG7glc5ibZqrA9OgyZPVBQ6U4HaCC6fgz5OGozBSjaAIaqtnfibea95S1zjmeXb1yvgw6J6STuzliaMHj0u2T9gCtLk2Lw/640?wx_fmt=png&from=appmsg#imgIndex=5)

LLM Wiki 不是一份"用 LLM 写出来的文档"，而是一份结构化、有约束、可验证的知识资产。它和传统文档的区别在四个层面：

- 结构可解析——单页内部结构。每个页面是 frontmatter + 正文的双层结构，frontmatter 是脚本可解析的 YAML，承载关系字段（type、domain、upstream、computed\_by 等），正文承载语义描述。脚本可直接读取关系信息，不必依赖 LLM 反复解析正文。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1yHibia2pr0cdC9IEU8cHaT2bPSds90KbAToCZvA2rmQxKVYrTZqwxeia1EHtsS6w8icSNHbekFkx9j9CictDk561Jm0uTtUCf8xNWc/640?wx_fmt=png&from=appmsg#imgIndex=6)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1zl0afsG4PYdUlG9hq3LCI9AAr7MUkQeBFq5AHWrJwzVF7lyQg3tum1FeGXYwz1YS1KdHlYQm3icAyW0ibjlvUfLPib3rqDLJHvro/640?wx_fmt=png&from=appmsg#imgIndex=7)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1z143yTED0XiadAvHVIooMx0WNhicHEAusIlk4vpUiaCCyfCO72PU440DibHjJLT0CHCQUcnrJ6dSNqnbmPaplFydVr912rxhIthYI/640?wx_fmt=png&from=appmsg#imgIndex=8)
...
- 主流程目录承载知识从源到产物的状态流： `pre/` 维护待处理清单，并临时承接抓取下来的原始材料，验证通过后移入 `raw/` ； `raw/` 按状态分流存放原始材料，供下游编译使用； `wiki/` 存放编译完成的结构化页面——清单 → 原始材料 → 结构化页面，三态接力构成完整的主路径。
- 支撑目录服务于编译过程本身： `log/` 提供可观测性（构建日志、健康检查报告）， `tmp/` 承载跨 skill 协作的中间状态， `schema/` 定义页面契约（frontmatter 与正文模板）。两类目录互不干扰，主流程跑通即可对外发布。
...
每个页面是 frontmatter + 正文的双层结构，两者通过 Schema 绑定，形成统一契约。frontmatter 是 YAML 格式的结构化头部，包含共性字段（title、type、description、sources、created、updated 等所有页面都有的字段）和页面特有字段（如 table 的 upstream/downstream/domain/layer，metric 的 computed\_by，dashboard 的 uses\_datasets 等）。这些字段承载关系和元数据，脚本可直接按字段提取，不需要解析自然语言。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1yOq0Ohk8kpaPVictfEFeqsRpBX3NestYvCFFKIpvNSpG4ibCNCpkOkHugxL4UOicqZC2bd8GXUTFGxI6ncoXXC6PYsHmvR6xg5kM/640?wx_fmt=png&from=appmsg#imgIndex=9)
...
| 边类型 | 含义 | 方向 | 来源字段 |
| --- | --- | --- | --- |
| `has_upstream` | ODPS 离线血缘 | 当前表 → 上游表 | `upstream` （task\_type=odps） |
| `syncs_from` | 同步任务血缘 | 当前表 → 来源表 | `upstream` （task\_type=sync） |
| `writes_from` | Flink 实时血缘 | 当前表 → 来源表 | `upstream` （task\_type=flink） |
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1whDYfSob7lvGVprYKbs44BxDHw5SemhhF3kcKYObpW0ksdmZkFmMtOTwfHbd0EAgYfmKFhKeC2cQnGmTtOgpFSgbrgaZYXmBk/640?wx_fmt=png&from=appmsg#imgIndex=10)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1x2QI19lcQiaH6tJltiadbAHEQ6TxuEicMR3LgEo2kmstVKp0FjwmV1AnaljniaShJs3t8B9nShE2Ma0DpGMG9o2qX3OR0VpyDiardo/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=2)
